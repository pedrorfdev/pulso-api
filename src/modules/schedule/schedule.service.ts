...
export class ScheduleService {
  ...
  async createEvent(
    ...
    // deadline no passado é permitido — o evento pode ser criado em cima da hora
    // o cron job de deadline só processa attendances PENDING, não bloqueia criação
  ...
  async publishEvent(eventId: string, orgId: string): Promise<EventResponse> {
    const event = await this.findEventInOrg(eventId, orgId);
    ...
    if (slots.length === 0) {
      throw new BadRequestError(
        "Adicione ao menos um membro antes de publicar",
      );
    }
  ...
  async addSlot(
    ...
    if (event.is_published) {
      throw new BadRequestError(
        "Não é possível alterar escala de evento publicado",
      );
    }

    // aceita tanto o id do OrganizationMember quanto o user_id
    // (o front pode mandar qualquer um dos dois dependendo do contexto)
    let member = await this.db.organizationMember.findFirst({
      where: {
        organization_id: orgId,
        OR: [{ id: data.member_id }, { user_id: data.member_id }],
      },
    });
    ...
    if (existingSlot) {
      ...
      const newRoleLabels =
        data.mode === "add"
          ? Array.from(
              new Set([...existingSlot.role_labels, ...data.role_labels]),
            )
          : data.role_labels;
      ...
      const updated = await this.db.scheduleSlot.update({
        ...
        data: {
          role_labels: newRoleLabels,
          notes: data.notes ?? existingSlot.notes,
        },
        include: {
          member: {
            include: {
  ...
  async removeSlot(slotId: string, orgId: string): Promise<void> {
    ...
    if (slot.event.is_published) {
      throw new BadRequestError(
        "Não é possível remover slot de evento publicado",
      );
    }
