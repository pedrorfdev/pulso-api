...
export class SongService {
  ...
  async deleteSong(songId: string, orgId: string): Promise<void> {
    const song = await this.db.song.findFirst({
      where: { id: songId, organization_id: orgId },
    });
    if (!song) throw new NotFoundError("Song");
    await this.db.song.delete({ where: { id: songId } });
  }

  async listEventSongs(
    eventId: string,
    orgId: string,
  ...
  async addSongToEvent(
    ...
    const song = await this.db.song.findFirst({
      where: { id: data.song_id, organization_id: orgId },
    });
    if (!song) throw new NotFoundError("Song");
    ...
    return {
      ...
      notes: eventSong.notes,
      song: this.toResponse(eventSong.song),
    };
  }
